...
def build_facts_summary(events: list[dict]) -> str:
    """Build a 7-day table view of events, one row per day."""
    local_tz = tz.gettz(TIMEZONE)
    now = datetime.now(tz=local_tz)
    ...
    for e in events:
        ...
        time_str = dt.strftime("%H:%M") if "T" in e["start"] else "All-day"
        src_emoji = {"me": "👤", "partner": "👫", "family": "👨‍👩‍👧"}.get(e["source"], "📅")
        title = e["summary"] or "(No title)"
        events_by_date[date_key].append(f"{time_str} {src_emoji} {title}")
    ...
    for i in range(7):
        ...
        if day_events:
            events_str = " │ ".join(day_events[:5])
            if len(day_events) > 5:
                events_str += f" (+{len(day_events) - 5} more)"
        else:
            events_str = "—"
        
        lines.append(f"{day_label:<12} {events_str}")
...
def send_email_gmail(to_email: str, subject: str, body_html: str, body_text: str = ""):
    gmail = get_gmail_service()

    msg = EmailMessage()
    msg["To"] = to_email
    msg["Subject"] = subject
    msg.set_content(body_text or "Please view this email in HTML mode.")
...
def build_calendar_html(events: list[dict]) -> str:
    """Build an HTML table for the 7-day calendar view."""
    local_tz = tz.gettz(TIMEZONE)
    now = datetime.now(tz=local_tz)
    ...
    for e in events:
        ...
        time_str = dt.strftime("%H:%M") if "T" in e["start"] else "All-day"
        src_emoji = {"me": "👤", "partner": "👫", "family": "👨‍👩‍👧"}.get(e["source"], "📅")
        src_color = {"me": "#3b82f6", "partner": "#ec4899", "family": "#22c55e"}.get(e["source"], "#6b7280")
        title = e["summary"] or "(No title)"
    ...
    for i in range(7):
        ...
        if day_events:
            events_html = "".join([
                f'<div style="margin: 4px 0; padding: 6px 10px; background: {ev["color"]}15; '
                f'border-left: 3px solid {ev["color"]}; border-radius: 4px;">'
                f'<span style="color: #666; font-size: 12px;">{ev["time"]}</span> '
                f'{ev["emoji"]} <strong>{ev["title"]}</strong></div>'
                for ev in day_events[:5]
            ])
            if len(day_events) > 5:
                events_html += f'<div style="color: #999; font-size: 12px;">+{len(day_events) - 5} more</div>'
        else:
            events_html = '<div style="color: #ccc; font-style: italic; padding: 6px 0;">No events</div>'
        
        bg_color = "#f8fafc" if i % 2 == 0 else "#ffffff"
        rows.append(
            f'<tr style="background: {bg_color};">'
            f'<td style="padding: 12px; vertical-align: top; width: 100px; border-bottom: 1px solid #e5e7eb;">'
            f'<strong style="color: #1f2937;">{day_name}</strong><br>'
            f'<span style="color: #6b7280; font-size: 13px;">{day_date}</span></td>'
            f'<td style="padding: 12px; border-bottom: 1px solid #e5e7eb;">{events_html}</td></tr>'
        )
    
    return f'<table style="width: 100%; border-collapse: collapse; font-family: -apple-system, sans-serif;">{"".join(rows)}</table>'


def node_email_body(state: State) -> State:
    local_tz = tz.gettz(TIMEZONE)
    now = datetime.now(tz=local_tz)
    generated_time = now.strftime("%A %d %b %Y, %H:%M")
    
    start_date = now
    end_date = now + timedelta(days=6)
    date_range = f"{start_date.strftime('%A %B %-d')} – {end_date.strftime('%A %B %-d, %Y')}"
    
    trello_section = state.get("trello_result", "")
    trello_html = ""
    if trello_section and "disabled" not in trello_section.lower() and "skipped" not in trello_section.lower():
        trello_html = f'''
        <div style="margin-top: 30px; padding: 20px; background: #fef3c7; border-radius: 12px; border-left: 4px solid #f59e0b;">
            <h2 style="margin: 0 0 15px 0; color: #92400e; font-size: 18px;">📋 Trello</h2>
            <pre style="margin: 0; white-space: pre-wrap; font-family: inherit; color: #78350f;">{trello_section}</pre>
        </div>'''
    
    recommendations_formatted = state["recommendations"].replace("\n", "<br>")
...
<body style="margin: 0; padding: 20px; background: #f3f4f6; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
    <div style="max-width: 650px; margin: 0 auto; background: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
        
        <!-- Header -->
        <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 30px; text-align: center;">
            <h1 style="margin: 0; color: white; font-size: 28px; font-weight: 600;">📅 Weekly Digest</h1>
            <p style="margin: 10px 0 0 0; color: rgba(255,255,255,0.9); font-size: 16px;">{date_range}</p>
            <p style="margin: 5px 0 0 0; color: rgba(255,255,255,0.7); font-size: 12px;">Generated {generated_time}</p>
        </div>
        
        <div style="padding: 25px;">
            
            <!-- Weather Section -->
            <div style="margin-bottom: 30px; padding: 20px; background: linear-gradient(135deg, #e0f2fe 0%, #f0f9ff 100%); border-radius: 12px;">
                <h2 style="margin: 0 0 15px 0; color: #0369a1; font-size: 18px;">🌤️ Weather This Week</h2>
                <pre style="margin: 0; white-space: pre-wrap; font-family: inherit; color: #0c4a6e; font-size: 15px; line-height: 1.6;">{state["weather_summary"]}</pre>
            </div>
            
            <!-- Calendar Section -->
            <div style="margin-bottom: 30px;">
                <h2 style="margin: 0 0 15px 0; color: #1f2937; font-size: 18px;">📆 Calendar</h2>
                <div style="border: 1px solid #e5e7eb; border-radius: 12px; overflow: hidden;">
                    {build_calendar_html(state["events"])}
                </div>
            ...
            <div style="padding: 20px; background: linear-gradient(135deg, #fef3c7 0%, #fffbeb 100%); border-radius: 12px; border-left: 4px solid #f59e0b;">
                <h2 style="margin: 0 0 15px 0; color: #92400e; font-size: 18px;">✨ Recommendations</h2>
                <div style="color: #78350f; line-height: 1.7;">{recommendations_formatted}</div>
            </div>
        ...
        <div style="padding: 20px; background: #f9fafb; text-align: center; border-top: 1px solid #e5e7eb;">
            <p style="margin: 0; color: #9ca3af; font-size: 12px;">
                🏠 {LAUSANNE["name"]} • Powered by The Lausanne Guide
            </p>
        </div>
